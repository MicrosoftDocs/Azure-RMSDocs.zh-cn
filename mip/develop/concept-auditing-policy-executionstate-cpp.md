...
每个属性都由应用进行设置，然后 `ExecutionSateOptions` 会被传递给派生自 `mip::ExecutionState` 的类的构造函数。 此信息用于确定要执行的操作。 `mip::ExecutionState` 中提供的数据也将显示在 Azure 信息保护分析中。

### <a name="next-steps"></a>后续步骤

接下来，了解有关[计算操作](concept-auditing-policy-computeactions-cpp.md)的详细信息。
