...
public std：： vector\<std：： string\> GetRights （） const  |  获取为与此 ProtectionHandler 关联的用户/标识授予的权限。
...
如果要用此 ProtectionHandler 加密内容，则计算内容的大小（以字节为单位）。
...
获取为与此 ProtectionHandler 关联的用户/标识授予的权限。
