---
title: MIP SDK forC++引用结构和枚举
description: MIP 的参考文档C++SDK 结构和枚举。
author: msmbaldwin
...
自动            | 由策略条件设置
建议            | 标签建议的策略条件后，由用户设置
DEFAULT 的信任组中的所有虚拟机的复制设置            | 默认情况下，在策略中设置
必需            | 策略强制设置标签后由用户设置
...
内容格式。
 ...
 值                         | 说明                                
--------------------------------|---------------------------------------------
DEFAULT 的信任组中的所有虚拟机的复制设置            | 内容格式是标准文件格式
电子邮件            | 内容格式是电子邮件格式
...
ADHOC_PROTECTION_REQUIRED            | 临时保护应设置为完成文件上的操作
...
##### <a name="applicationid-struct-member"></a>applicationId 结构成员
作为应用程序标识符的 AAD 门户中设置，（应为不带方括号的 GUID）。
  
##### <a name="applicationname-struct-member"></a>applicationName 结构成员
应用程序名称 (只应包含有效 ASCII 字符不包括;)
  
##### <a name="applicationversion-struct-member"></a>applicationVersion 结构成员
正在使用的应用程序的版本 (只应包含有效 ASCII 字符不包括;)  
...
##### <a name="licenseinfo-struct-member"></a>licenseInfo 结构成员
_尚无记录。_

  
##### <a name="serializedpublishinglicense-struct-member"></a>serializedPublishingLicense 结构成员
_尚无记录。_

  
##### <a name="publishinglicensecontext-function"></a>PublishingLicenseContext 函数
_尚无记录。_
