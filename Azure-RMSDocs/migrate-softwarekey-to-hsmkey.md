...
必须先从 AD RMS 配置数据中提取服务器许可方证书（SLC）密钥，然后将该密钥传输到本地 nCipher HSM，接下来，将 HSM 密钥传输到 Azure Key Vault，然后授权 azure 信息保护中的 Azure Rights Management 服务访问密钥保管库，然后导入配置数据。
