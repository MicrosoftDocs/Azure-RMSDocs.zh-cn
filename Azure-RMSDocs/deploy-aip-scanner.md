...
|存储扫描程序配置的 SQL Server：<br /><br />- 本地或远程实例<br /><br />- [不区分大小写排序规则](https://docs.microsoft.com/sql/relational-databases/collations/collation-and-unicode-support?view=sql-server-ver15) <br /><br />用于安装扫描程序的 Sysadmin 角色|SQL Server 2012 是以下版本的最低版本：<br /><br />- SQL Server Enterprise<br /><br />- SQL Server Standard<br /><br />- SQL Server Express<br /><br />当你为扫描程序指定自定义群集（配置文件）名称时，Azure 信息保护扫描程序在相同的 SQL server 实例上支持多个配置数据库。 当你使用来自统一标签客户端的扫描程序预览版本时，多个扫描仪可以共享同一个配置数据库。<br /><br />如果安装扫描程序且帐户拥有 Sysadmin 角色，那么在安装过程中会自动创建扫描程序配置数据库，并向运行扫描程序的服务帐户授予所需的 db_owner 角色。 如果无法获得 Sysadmin 角色或组织策略要求手动创建和配置数据库，请参阅[使用备用配置部署扫描程序](#deploying-the-scanner-with-alternative-configurations)部分。<br /><br />有关容量指导，请参阅[SQL Server 的存储要求和容量规划](#storage-requirements-and-capacity-planning-for-sql-server)。|
...
- **磁盘大小**：对于每个部署，配置数据库的大小会有所不同，但建议你为要扫描的每个1000000文件分配 500 MB。
...
- **对于多个扫描仪，最多 10**个：4核处理器;建议 8 GB RAM。

- **对于超过10个（最多40个）的多个扫描仪**：8个核心进程;建议使用 16 GB RAM。
...
- 对于经典客户端：对[断开连接的计算机的支持](./rms-client/client-admin-guide-customizations.md#support-for-disconnected-computers)
...
1. 通过创建扫描仪群集，在 Azure 门户中配置扫描仪。 如果需要此步骤的帮助，请参阅[在 Azure 门户中配置扫描程序](#configure-the-scanner-in-the-azure-portal)。
...
- 对于经典客户端：
    ...
    如果没有为扫描仪指定自己的群集（配置文件）名称，则配置数据库将命名**AIPScanner_ \<computer_name> **（仅限经典客户端）。 继续执行以下步骤以创建用户并授予对数据库的 db_owner 权限。 

- 对于统一标签客户端：
    
    如果指定自己的群集（配置文件）名称，则配置数据库将命名**AIPScannerUL_<cluster_name>** （统一的标记客户端）。
...
1. 为扫描程序创建数据库： <br>
    **CREATE database AIPScannerUL_ [clustername]** **ALTER database AIPScannerUL_ [CLUSTERNAME] SET 高可信 ON**
...
- 对于 Azure Active Directory 帐户，请使用以下说明：
    - 对于经典客户端：请参阅在该客户端管理员指南中[指定和使用 set-aipauthentication 的 Token 参数](./rms-client/client-admin-guide-powershell.md#specify-and-use-the-token-parameter-for-set-aipauthentication)。
    - 对于统一标签客户端：请在该客户端管理员指南中的[如何在 Azure 信息保护中以非交互方式标记文件](./rms-client//clientv2-admin-guide-powershell.md#how-to-label-files-non-interactively-for-azure-information-protection)中所述，为具有 Set-aipauthentication 的*OnBehalfOf*参数指定你的本地帐户。


## <a name="configure-the-scanner-in-the-azure-portal"></a>在 Azure 门户中配置扫描程序

在安装扫描程序之前，或者从扫描程序的旧版本升级它，请在 Azure 门户中为扫描仪创建群集和内容扫描作业。 为扫描程序设置和要扫描的数据存储库配置群集和内容扫描作业。
...
2. 找到 "**扫描仪**" 菜单选项，然后选择 "**群集**"。

3. 在 " **Azure 信息保护-群集**" 窗格上，选择 "**添加**"：
    
    ![为 Azure 信息保护扫描程序添加内容扫描作业](./media/scanner-add-profile.png)

4. 在 "**添加新群集**" 窗格上，指定用于标识其配置设置和要扫描的数据存储库的扫描仪的名称。 例如，可以指定“欧洲”**** 来标识扫描程序将涵盖的数据存储库的地理位置。 以后安装或升级扫描程序时，需要指定相同的群集名称。
...
8. 既然已创建并保存了内容扫描作业，你就可以返回到 "**配置存储库**" 选项来指定要扫描的数据存储。 你可以指定本地文件夹、UNC 路径以及 sharepoint 本地文档库和文件夹的 SharePoint Server Url。 
...
10. 如果要添加另一个数据存储库，请重复步骤8和9。 

11. 你现在可以关闭 "**存储库**" 窗格和 "**内容扫描作业**" 窗格。 返回 " **Azure 信息保护-内容扫描作业**" 窗格，显示你的内容扫描名称，以及显示为 "**手动**" 和 "**强制**" 列为空的 "**计划**" 列。

你现在已准备好将扫描程序与刚刚创建的内容扫描程序作业一起安装。
...
3. 运行[install-aipscanner](/powershell/module/azureinformationprotection/Install-AIPScanner) cmdlet，指定要在其中为 Azure 信息保护扫描程序创建数据库的 SQL Server 实例，以及在上一节中指定的扫描仪群集名称： 
...
4. 使用**管理工具**服务验证是否已安装该服务  >  **Services**。 
...
2. 等待扫描程序完成其周期。 当扫描程序浏览完指定数据存储中的所有文件时，扫描程序停止（尽管扫描程序服务仍在运行）：
    ...
    - 在 " **Azure 信息保护-内容扫描作业**" 窗格上，使用 "**刷新**" 选项，并等待，直到看到 "**最后一次扫描结果**" 列和 "**上次扫描（结束时间）** " 列的值。
    
    - 使用 PowerShell，你可以运行 `Get-AIPScannerStatus` 来监视状态更改。
    
    - 仅限经典客户端的扫描程序：检查本地 Windows**应用程序和服务**事件日志和**Azure 信息保护**。 另外，此日志还会报告扫描程序完成扫描的时间，以及结果摘要。 请查看信息事件 ID 911****。
...
如果按照这些说明操作，扫描程序在仅报告模式下运行一次。 若要更改这些设置，请编辑内容扫描作业：

1. 返回到 " **Azure 信息保护-内容扫描作业**" 窗格，选择要编辑的群集和内容扫描作业。

2. 在 \<**content scan job name**> 窗格中，更改以下两个设置，然后选择 "**保存**"：
...
4. 仅限经典客户端的扫描程序：再次监视信息类型为**911**的事件日志，时间戳晚于在上一步中开始扫描的时间。
...
可以定义要扫描或不要扫描的文件类型列表，从而更改此行为。 默认情况下，可以为扫描程序指定此列表以应用于所有数据存储库，并且可以为每个数据存储库指定一个列表。 若要指定此列表，请使用内容扫描作业中的 "**文件类型" 扫描**设置：
...
- 如果你希望 Azure 信息保护客户端使用与 Azure 信息保护客户端相同的默认保护行为，其中所有文件都自动使用本机保护或常规保护进行保护：将通配符指定为 `*` 注册表项， `Encryption` 作为值（REG_SZ），并将 `Default` 作为值数据。
...
你可以强制扫描程序再次从 Azure 门户中的 " **Azure 信息保护-内容扫描作业**" 窗格检查所有文件。 从列表中选择内容扫描作业，然后选择 "**重新扫描所有文件**" 选项：
...
此外，当经典客户端的扫描程序下载包含新的或更改的条件的 Azure 信息保护策略时，将检查所有文件，并且来自统一标签客户端的扫描程序具有新的或更改的自动标记和建议标记的设置。 
...
对于已添加到内容扫描作业的数据存储库，可以使用**导出**和**导入**选项来快速更改设置。 例如，对于 SharePoint 数据存储库，你希望添加要从扫描中排除的新文件类型。
...
- 将默认标签应用于数据存储库中的所有文件。
    ...
    不会检查文件的内容，并且数据存储库中的所有未标记文件都将根据你为数据存储库或内容扫描作业指定的默认标签进行标记。 
...
- 标识所有自定义条件和已知敏感信息类型。
    ...
    对于经典客户端的扫描程序：扫描程序使用你为 Azure 信息保护策略中的标签指定的任何自定义条件，以及可用于为 Azure 信息保护策略中的标签指定的信息类型的列表。 
...
如有必要，请安装扫描程序的多个实例。 当你为扫描程序指定自定义群集（配置文件）名称时，Azure 信息保护扫描程序在相同的 SQL server 实例上支持多个配置数据库。 对于来自统一标签客户端的扫描程序，多个扫描程序可以共享同一群集（配置文件），这会缩短扫描时间。
...
因为现在已经从 Azure 门户配置了扫描仪，所以以前版本中的 cmdlet 用于配置数据存储库，而已不推荐使用已扫描的文件类型列表。 

还包括 cmdlet，这些 cmdlet 用于安装和升级扫描程序、更改扫描程序配置数据库和群集（配置文件）、更改本地报表级别以及导入断开连接的计算机的配置设置。 
...
如果扫描程序被配置为手动运行而不是连续运行，则要再次扫描文件，请将 "**计划**" 设置为 "**手动**" 或 "**始终**在内容扫描作业中"，然后重新启动该服务。
