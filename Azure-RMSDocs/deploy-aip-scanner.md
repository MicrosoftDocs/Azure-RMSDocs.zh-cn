...
2. 在 " \<**内容扫描作业名称**>" 窗格上，更改以下两个设置，然后选择 "**保存**"：
...
- 如果你希望 Azure 信息保护客户端使用与 Azure 信息保护客户端相同的默认保护行为，其中所有文件都自动使用本机保护`*`或常规保护进行保护： `Encryption`将通配符指定为注册表项，作为`Default`值（REG_SZ），并将作为值数据。
