...
- 对于统一标签客户端：[支持断开连接的计算机](./rms-client/clientv2-admin-guide-customizations.md#support-for-disconnected-computers)
    
    在此配置中，通过将*DelegatedUser*参数与[set-aipauthentication](/powershell/module/azureinformationprotection/set-aipauthentication) cmdlet 结合使用，来自统一标签客户端的扫描程序可以应用保护、删除保护和检查受保护的文件。
...
- 如果没有为扫描仪指定自己的配置文件名称（仅限经典客户端），则配置数据库的名称为**AIPScanner_\<** 计算机名 >。 

- 如果指定自己的配置文件名称，则配置数据库名为**AIPScanner_\<profile_name >** （经典客户端）或**AIPScannerUL_\<profile_name >** （统一的标记客户端）。
...
2. 在 Windows Server 计算机中，如果你的扫描程序服务帐户已就安装授予了“本地登录”权限：使用此帐户登录并启动 PowerShell 会话。 运行 [Set-AIPAuthentication](/powershell/module/azureinformationprotection/set-aipauthentication)，指定从上一步骤中复制的值：
    ...
    **对于统一标签客户端：**
    ...
    如果无法将扫描仪服务帐户授予**本地登录**的权限，请使用 Set-aipauthentication 的*OnBehalfOf*参数，如[如何为 Azure 信息以非交互方式标记文件中所述。](./rms-client//clientv2-admin-guide-powershell.md#how-to-label-files-non-interactively-for-azure-information-protection)从该客户端管理员指南中的保护。
...
- 如果你希望 Azure 信息保护客户端使用与 Azure 信息保护客户端相同的默认保护行为，其中所有文件都自动使用本机保护或常规保护进行保护：将 `*` 通配符指定为注册表项，`Encryption` 为值（REG_SZ），并 `Default` 作为值数据。
...
>
> - 来自统一标签客户端的扫描程序：从%LocalAppData%\Microsoft\MSIP\mip 中手动删除内容 **<> \mip\\** 。
>
