...
使用 Azure 门户，管理员可为作用域内策略和标签配置中的保护设置指定电子邮件地址。 有关详细信息，请参阅[如何使用作用域内策略为特定用户配置 Azure 信息保护策略](../deploy-use/configure-policy-scope.md)以及[如何为 Rights Management 保护配置标签](configure-policy-protection.md)。 

对于配置为从 Azure Rights Management 服务应用保护的标签，还可以在保护模板中找到电子邮件地址，方法是从 [AADRM 模块](/powershell/module/aadrm)使用 PowerShell cmdlet。 此 PowerShell 模块还允许管理员按照电子邮件地址将用户指定为[超级用户](../deploy-use/configure-super-users.md)，或 Azure Rights Management 服务的管理员。 
...
可以为 Azure 信息保护策略中的作用域内策略和保护设置更新电子邮件地址。 有关详细信息，请参阅[如何使用作用域内策略为特定用户配置 Azure 信息保护策略](../deploy-use/configure-policy-scope.md)以及[如何为 Rights Management 保护配置标签](configure-policy-protection.md)。 
...
可以删除 Azure 信息保护策略中作用域内策略和保护设置的电子邮件地址。 有关详细信息，请参阅[如何使用作用域内策略为特定用户配置 Azure 信息保护策略](../deploy-use/configure-policy-scope.md)以及[如何为 Rights Management 保护配置标签](configure-policy-protection.md)。 
