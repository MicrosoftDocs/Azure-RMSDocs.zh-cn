...
此标签不可限制到 Outlook，但可提供限制性低于“不要转发”的控制。 例如，希望收件人能够复制电子邮件或附件，或者打印和保存附件。 如果指定没有 Azure AD 帐户的外部用户，请确保告知用户此标签仅可用于电子邮件，不得用于文档。 此外，要支持这些外部用户，还须为 Exchange Online 配置 [Office 365 邮件加密新功能](https://support.office.com/article/7ff0c040-b25c-4378-9904-b1b50210d00e)。  

用户在“收件人”框中指定电子邮件地址时，该地址必须与为此标签配置指定的用户地址相同。 因为用户可能属于组并且拥有多个电子邮件地址，所以他们指定的电子邮件地址不必与你为权限指定的电子邮件地址完全匹配，虽然这是确保成功对收件人授权的最简单方法。 要详细了解如何向用户授予权限，请参阅[准备用户和组以便使用 Azure 信息保护](../plan-design/prepare.md)。 

1. 在“保护”边栏选项卡中，确保选中“Azure (云密钥)”。
    
2. 务必选中“设置权限”，然后选择“添加权限”。

3. 要向组织中的用户授予权限，请在“添加权限”边栏选项卡上选择“添加 \<组织名称> - 所有成员”以选择租户中所有用户，或选择“浏览目录”以选择特定组。 要向外部用户授予权限，请选择“输入详细信息”，然后键入用户或 Azure AD 组的电子邮件地址。
