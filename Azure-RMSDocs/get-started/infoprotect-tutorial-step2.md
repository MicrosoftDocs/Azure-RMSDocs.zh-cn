...
3. 注意：首次连接到该服务时，系统将自动打开“快速入门”页上的信息。 你可以稍后返回到此信息。 在本教程中，单击“全局策略”以打开“策略: 全局”边栏选项卡。 此边栏选项卡将自动打开，以便执行后续连接到服务的操作，它将显示自动为租户创建的默认信息保护策略：
    ...
    - 用于分类的标签：“个人”、“公共”、“常规”、“机密”和“高度机密”。 最后两个标签可展开以显示包括“所有员工”和“任何人(不受保护)”的子标签，并提供有关如何使分类具有子类别的示例。
    
       > [!NOTE]
       > 你的默认策略可能与本教程中的默认策略稍有不同。 例如，你有名为“内部”的标签，而没有“常规”标签，有“秘密”标签而没有“高度机密”标签。 或者具有名为“仅收件人”的额外子标签。 这是因为存在不同版本的默认策略，具体取决于为租户创建策略的时间。 或者在开始本教程之前，已自行对其进行编辑。
       > 
       > 如果你的默认策略不同，仍可使用本教程，但使用其中的说明和图片时，应注意这些更改。 如果要修改自己默认策略以符合当前的默认策略，请参阅[默认 Azure 信息保护策略](../deploy-use/configure-policy-default.md)。

    - 在默认配置中，某些标签未配置视觉标记（例如页脚、页眉、水印）。 某些标签可能设置了保护或没有设置保护，具体取决于默认策略。 例如：
    
    ![Azure 信息保护快速入门教程步骤 3 - 默认策略](../media/info-protect-policy-default-labelsv2.png)
    
    此外，未设置某些策略设置。 并非所有文档和电子邮件都需要标签，没有默认标签，用户更改标签时无需提供理由：
...
如果因使用旧版本的策略而导致“机密”标签不具有子标签，则可以改用“机密”标签。 配置步骤相同，但标签边栏选项卡的名称是“机密”，而不是“所有员工”。

1. 请确保展开“机密”标签以显示子标签，然后为“所有员工”标签记录是否为“保护”列显示“Azure RMS”。 如果是，则将自动配置此标签的最新默认策略和保护。 如果此列为空白，则需要在稍后的步骤中配置保护。

    选择此“所有员工”子标签，在新的“标签: 所有员工”边栏选项卡中，将看到可用于每个标签的设置。 

2. 读取此标签的“说明”文本。 该文本描述应如何使用所选标签，并显示为一个工具提示，帮助用户确定要选择的标签。

3. 如果已为标签配置保护，请转到步骤 5。
    
    如果未为标签配置保护，请找到“为包含此标签的文档和电子邮件设置权限”部分。 选择“保护”，然后选择“保护”栏：
    
    ![为 Azure 信息保护标签配置的保护](../media/info-protect-protection-bar-configured.png) 
    
4. 在“保护”边栏选项卡中，请务必选中“Azure RMS”，并选择“选择预定义模板”。 单击下拉列表框并选择默认模板，以便组织中的所有用户可以查看和编辑受保护的内容。 
...
4. 单击“确定”保存所做的更改，这将关闭“保护”边栏选项卡。 可看到在“标签: 所有员工”边栏选项卡中更新的“保护”栏。 例如：
...
5. 在“标签: 所有员工”边栏选项卡上，找到“设置可视标记”部分：
    ...
    尽管可以更改水印的大小、颜色和布局，但是我们将暂时使用这些设置的默认值。
...
10. 在此“标签: 所有员工”边栏选项卡上，单击“保存”。 然后在“策略: 全局”边栏选项卡上，再次单击“保存”。
    ...
    如果已为标签配置保护，则标签现已更新，以显示 Azure RMS 保护：
...
|默认策略中的配置设置|[默认 Azure 信息保护策略](../deploy-use/configure-policy-default.md)|
