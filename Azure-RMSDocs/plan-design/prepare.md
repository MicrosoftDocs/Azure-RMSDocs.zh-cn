...
使用此列表中的前三种方法创建用户和组时，它们将在 Azure AD 中自动创建，Azure 信息保护可以直接使用这些帐户。 然而，许多企业网络使用本地目录来创建和管理用户和组。 Azure 信息保护不能直接使用这些帐户；必须将它们同步到 Azure AD。

## <a name="how-users-and-groups-are-used-by-azure-information-protection"></a>Azure 信息保护如何使用用户和组

Azure 信息保护使用用户和组的方式有三种：

- 使用标签和分类时**用于向用户分配标签**。 只有管理员可以选择这些组：
...
- 若要配置将其他标签分配给组成员的作用域策略，可以使用 Azure AD 中的任何类型的组，但需要具有包含用户租户的已验证域的电子邮件地址。 具有电子邮件地址的组通常称为启用邮件的组。
    ...
    例如，可以使用启用邮件的安全组、通讯组（可以是静态或动态）和 Office 365 组。 不能使用安全组（动态或静态），因为该组类型没有电子邮件地址。
...
若要将本地托管的帐户用于 Azure 信息保护，必须将其同步到 Azure AD。 为方便部署，我们建议使用 [Azure AD Connect](/azure/active-directory/connect/active-directory-aadconnect)。 但是，可以使用可实现相同结果的任何目录同步方法。
...
确认用户和组可用于 Azure 信息保护，并已准备好开始保护文档和电子邮件时，激活 Rights Management 服务来启用此数据保护服务。 如果尚未这样做，请参阅[激活 Azure Rights Management](../deploy-use/activate-service.md)。
