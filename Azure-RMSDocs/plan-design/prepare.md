...
- 你可以从租户的已验证域使用 Azure AD 中任何类型的组（无论是否具有电子邮件地址），以进行 Azure 权限管理服务的委派管理。
...
确认用户和组可用于 Azure 信息保护，并已准备好开始保护文档和电子邮件时，激活权限管理服务来启用此数据保护服务。 有关详细信息，请参阅[激活 Azure Rights Management](../deploy-use/activate-service.md)。
