...
如果 Windows 计算机运行的 Office 365 应用程序的最低版本为1910，并且安装了一个 Azure 信息保护客户端，则默认情况下，Office 应用程序中会禁用内置的标记客户端。 但是，可以更改此行为，以便仅将内置标签客户端用于 Office 应用。 使用此配置时，Azure 信息保护客户端（经典或统一标签）仍可用于在文件资源管理器、PowerShell 和扫描程序中进行标记。 有关禁用 Office 365 应用中的 Azure 信息保护客户端的说明，请参阅 Office 文档中的 "在[适用于 Windows 的 Azure 信息保护客户端上运行敏感度标签](https://docs.microsoft.com/microsoft-365/compliance/sensitivity-labels-office-apps#can-sensitivity-labels-run-alongside-the-azure-information-protection-client-in-office-for-windows)" 部分。
...
- 还会自行安装统一标签客户端，以测试 Azure 信息保护扫描程序的预览版本。
