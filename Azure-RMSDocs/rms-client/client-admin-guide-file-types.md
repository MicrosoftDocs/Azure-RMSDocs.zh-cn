---
title: "Azure 信息保护客户端支持的文件类型 | Azure 信息保护"
...
|文件类型默认值|这是以下文件类型的默认保护级别：<br /><br />- 文本和图像文件<br /><br />- Microsoft Office（Word、Excel、PowerPoint）文件<br /><br />- 可移植文档格式 (.pdf)<br /><br />有关详细信息，请参阅以下部分：[支持的文件类型和文件扩展名](#supported-file-types-for-protection-and-their-file-name-extensions)。|这是针对不受完整保护支持的其他所有文件类型（例如 .vsdx、.rtf 等）的默认保护。|
...
### <a name="supported-file-types-for-protection-and-their-file-name-extensions"></a>支持保护的文件类型及其文件扩展名
下表列出了 Azure 信息保护客户端本机支持的文件类型。 对于这些文件类型，原始文件扩展名将在应用本机保护时发生变化，并且这些文件将变为只读形式。
...
下表列出了 Azure 信息保护客户端本机支持用于 Microsoft Office 应用程序的文件类型。 对于这些文件，在文件受 Rights Management 服务保护后，文件扩展名仍保持不变。
