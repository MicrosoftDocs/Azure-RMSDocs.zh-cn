...
description: "如果你了解或以前部署过 Active Directory Rights Management Services (AD RMS)，你可能想知道 Azure RMS 在功能和要求方面与它相比有何差别。"
author: cabailey
manager: mbaldwin
ms.date: 08/25/2016
ms.topic: article
...
>*适用于：Active Directory Rights Management Services、Azure Rights Management、Office 365*

如果你了解或以前部署过 Active Directory Rights Management Services (AD RMS)，你可能想知道 [!INCLUDE[aad_rightsmanagement_1](../includes/aad_rightsmanagement_1_md.md)] (Azure RMS) 在功能和要求方面与它相比有何差别。 

Azure RMS 的一些主要差异：

- **不需要服务器基础结构**：Azure RMS 不需要 AD RMS 所需的额外服务器和 PKI 证书，因为 Microsoft Azure 将处理这些内容。 因而这一云解决方案可以更快部署且更易维护。

- **基于云的身份验证**：对于内部用户和来自其他组织的用户，Azure RMS 都使用 Azure AD 进行身份验证。 这意味着即使移动用户未连接到你的内部网络，也可对其进行身份验证，并且你可以更轻松地与来自其他组织的用户共享受保护的内容。 许多组织由于运行 Azure 服务或使用 Office 365，因而在 Azure AD 中已拥有用户帐户。 但如果没有，用户可通过个人 RMS 创建免费帐户。 若要与其他组织共享受 AD RMS 保护的内容，则需要对每个组织配置显式信任。

- **对移动设备的内置支持**：无需进行部署更改，Azure RMS 即可支持移动设备和 Mac 计算机。 若要使 AD RMS 支持这些设备，必须安装移动设备扩展、配置 AD FS 以便进行联合身份验证，并为公共 DNS 服务创建其他记录。

- **默认模板**：激活 Azure RMS 服务后，它将创建两个默认模板，让你可以非常轻松地立即开始保护重要数据。 AD RMS 无默认模板。

- **部门模板**：Azure RMS 支持部门模板，它是你所创建额外模板的一项配置设置。 此设置允许指定可以在其客户端应用程序（例如 Office 应用）中看到该模板的用户，让他们能够更轻松地选择你为不同用户组选择定义的适当策略。 AD RMS 不支持部门模板。

- **文档跟踪、吊销和电子邮件通知**：Azure RMS 通过 RMS 共享应用支持这些功能，而 AD RMS 不支持。


此外，由于 Azure RMS 是一项云服务，因此与基于本地服务器的解决方案相比，它可以更快交付新功能和修补程序。 Windows Server 2016 中未为 AD RMS 规划新功能。

有关详细信息和其他差别，请使用以下表格并排比较 Azure RMS 和 AD RMS 的功能和优势。 如果你有安全方面的比较问题，请参阅本文中的[签名和加密的加密控制](compare-azure-rms-ad-rms.md#cryptographic-controls-for-signing-and-encryption)。

> [!NOTE]
> 为了简化这种比较，本文重复了 [Azure Rights Management 的要求](../get-started/requirements-azure-rms.md)中的一些信息。 使用该资源了解有关 [!INCLUDE[aad_rightsmanagement_1](../includes/aad_rightsmanagement_1_md.md)] 的更具体的支持和版本信息。
...
|可以在使用 RMS 共享应用程序时，通过[本机保护或常规保护](../rms-client/sharing-app-admin-guide-technical.md#levels-of-protection-native-and-generic)，保护所有文件类型。<br /><br />对于其他应用程序，请检查[《Azure RMS requirements: Applications》](../get-started/requirements-applications.md)（Azure RMS 要求：应用程序）中的表。|可以在使用 RMS 共享应用程序时，通过[本机保护或常规保护](../rms-client/sharing-app-admin-guide-technical.md#levels-of-protection-native-and-generic)，保护所有文件类型。<br /><br />对于其他应用程序，请检查[《Azure RMS requirements: Applications》](../get-started/requirements-applications.md)（Azure RMS 要求：应用程序）中的表。|
