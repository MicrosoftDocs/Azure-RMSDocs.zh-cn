...
若要为经典客户端配置分类、设置标签和保护，必须配置 Azure 信息保护策略。 然后将此策略下载到已安装[Azure 信息保护客户端](https://www.microsoft.com/download/details.aspx?id=53018)的计算机。
...
- 使用具有以下[管理员角色](/azure/active-directory/active-directory-assign-admin-roles-azure-portal)之一的 Azure AD 帐户：
  ...
  - **全局管理员**
    ...
    > [!NOTE] 
    > 如果你的租户位于[统一的标签平台](faqs.md#how-can-i-determine-if-my-tenant-is-on-the-unified-labeling-platform)上，则 Azure 门户不支持 Azure 信息保护管理员角色（以前称为 "信息保护管理员"）。 [详细信息](configure-policy-migrate-labels.md#administrative-roles-that-support-the-unified-labeling-platform)
