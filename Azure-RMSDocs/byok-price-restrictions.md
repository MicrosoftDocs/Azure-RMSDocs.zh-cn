...
> [!TIP]
> 如果需要，使用其他本地密钥向特定文档应用附加安全性。 有关详细信息，请参阅 [双重密钥加密 (DKE) 保护](plan-implement-tenant-key.md#double-key-encryption-dke) (仅) 的统一标签客户端。
>
> 如果你使用的是经典客户端并且需要额外的本地保护，则可以改为 [保留你自己的密钥 (HYOK) 保护](configure-adrms-restrictions.md) 保护。
> 
