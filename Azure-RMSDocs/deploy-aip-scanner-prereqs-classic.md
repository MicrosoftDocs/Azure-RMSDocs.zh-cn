...
> [!TIP] 
> 使用[教程](infoprotect-quick-start-tutorial.md)中的说明，使用一个在已准备好的 Word 文档中查找信用卡号的标签来测试扫描仪。 但是，你将需要更改标签配置，以使选项 "**选择应用此标签的方式**" 设置为 "**自动**"，而不是 "**建议**" 或 "将**建议标记视为自动**（在扫描仪版本 2.7. x. x. x. x 和更高版本中可用）"。 
>
