...
在该快速入门中，你将向 SharePoint 授予权限，让它能够扫描、安装和配置 Azure 信息保护扫描程序，来查找本地数据存储（例如网络共享和 SharePoint Server）中存储的文件中的敏感信息。
...
2. 计算机上已安装以下任一 Azure 信息保护客户端：
    ...
    - 经典客户端：要安装此客户端，请转到 [Microsoft下载中心](https://www.microsoft.com/download/details.aspx?id=53018)，然后从“Azure 信息保护”页下载 AzInfoProtection.exe。
    
    - 统一标签客户端：若要安装此客户端，请转到 [Microsoft下载中心](https://www.microsoft.com/download/details.aspx?id=53018)，然后从“Azure 信息保护”页下载 AzInfoProtection_UL.exe。
    
3. 计算机上同时安装了 SQL Server Express。
    
    如果尚未安装此 SQL Server 版本，可以从 [Microsoft 下载中心](https://www.microsoft.com/sql-server/sql-server-editions-express)进行下载，并选择基本安装。
...
8. 在“存储库”窗格上，指定在第一步中创建的文件夹。 例如：`\\server\TestScanner`
...
3. 只有一个小文件需要检查，因此初始测试扫描速度非常快：
    ...
    或者，仅对于经典客户端中的扫描程序：查看本地 Windows 应用程序和服务事件日志和 Azure 信息保护。 确认 MSIP.Scanner 进程的信息事件 ID 911。 事件日志条目还包含扫描结果的摘要。
...
1. 编辑扫描程序配置文件并添加新的数据存储库，这次指定要扫描敏感信息的本地数据存储。     
    可为 SharePoint 站点或库指定网络共享（UNC 路径）或 SharePoint Server URL。 
