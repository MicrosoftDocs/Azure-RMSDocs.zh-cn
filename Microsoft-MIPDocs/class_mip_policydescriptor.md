...
public void SetName | 设置策略名称。
public const std::string & GetDescription | 获取策略说明。
public void SetDescription | 设置策略说明。
public const std::vector< UserRights > & GetUserRightsList | 获取用户到权限映射的集合。
...
public void SetContentValidUntil | 设置策略过期时间。
public bool DoesAllowOfflineAccess | 获取策略是否允许离线访问内容。
public void SetAllowOfflineAccess | 设置策略是否允许离线访问内容。
public const std::string & GetReferrer | 获取策略引用网址。
public void SetReferrer | 设置策略引用网址。
public const AppDataHashMap & GetEncryptedAppData | 获取已加密的应用特定数据。
public void SetEncryptedAppDataAppDataHashMap & value) | 设置应加密的应用特定数据。
public const AppDataHashMap & GetSignedAppData | 获取已签名的应用特定数据。
public void SetSignedAppDataAppDataHashMap & value) | 设置应签名的应用特定数据。
## <a name="members"></a>成員
